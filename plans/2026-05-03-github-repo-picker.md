...
| `swarmer/templates/sessions/_repo_list.html` | Edit | Stable shell (`<div id="repo-list">`) with `hx-get="/repos/items"` + `hx-trigger="repoListChanged from:body"` + `hx-indicator="#repo-list-spinner"` — fetches fresh inner content whenever the event fires; includes `_repo_items.html` at initial page render; `#repo-list-spinner` bouncing-dot indicator shown during refresh |
| `swarmer/templates/sessions/_repo_items.html` | **New** | Inner partial: repo rows, delete buttons (`hx-swap="none"`), three-state add button; `#repo-picker-spinner` bouncing-dot indicator shown while picker list loads; served by `GET /repos/items` and swapped into `#repo-list` via `innerHTML` on `repoListChanged` |
| `swarmer/templates/sessions/detail.html` | Edit | `#repo-picker-container` placed as a sibling of `#repo-list` inside the card body |
| `swarmer/templates/base.html` | Edit | Added `@keyframes rbd-bounce` + `.repo-bounce-dots` CSS — three staggered dots (0 / 200 / 400 ms delay) bouncing up 5px with opacity pulse |
