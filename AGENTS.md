...
# OpenShell gateway (installs to current kubectl context — kind or OCP)
make openshell-setup     # Install Agent Sandbox CRDs + OpenShell Helm chart (idempotent)
make openshell-status    # Show installation status (Helm, pods, CRDs, TLS certs)
make openshell-delete    # Remove OpenShell and CRDs from current cluster
make openshell-extract-tls  # Re-extract mTLS client certs to auth/openshell/

# E2E tests (creates a unique ephemeral kind cluster, tears down after)
make test-e2e            # Full lifecycle: cluster → OpenShell → swarmer → e2e tests → teardown
```
...
2. **OpenShell requires Helm 3.8+**: `make openshell-setup` fails immediately with an upgrade link if Helm < 3.8 is detected (OCI chart support was added in 3.8). Current system Helm may be older — upgrade from https://helm.sh/docs/intro/install/ before running any openshell targets. mTLS client certs are extracted to `auth/openshell/` which is covered by the existing `auth/` gitignore entry.

3. **`auth.py` is dead code**: The file `swarmer/auth.py` contains only a comment "superseded by k8s_auth.py". All authentication logic is in `k8s_auth.py` and `routers/auth.py`.

4. **Deployment image placeholder**: `k8s/swarmer/deployment.yaml` uses literal strings like `SWARMER_IMAGE`, `OPENSHIFT_OAUTH_URL_VALUE`, `AGENT_IMAGE_OPENCODE_VALUE`, `AGENT_IMAGE_CRUSH_VALUE` which are replaced at deploy time via `sed` in the Makefile. Don't replace them with actual values.

5. **SQLite single-writer**: The K8s Deployment uses `strategy: Recreate` (not RollingUpdate) because SQLite doesn't support concurrent writers. Only one replica is safe.

6. **Session mode affects pod lifecycle**:
...
7. **OpenCode model format quirk**: Model strings use `provider/model@version` format (e.g., `google-vertex-anthropic/claude-sonnet-4-6@default`). The `@version` suffix is part of the model ID. Crush uses simpler `provider/model` format (e.g., `vertexai/claude-sonnet-4-6`).

8. **TUI auth tokens**: TUI WebSocket connections use one-time UUID tokens stored in the HTTP session. Tokens are generated on the session detail page and consumed on WebSocket connect. Invalid/reused tokens are rejected with close code 4001.

9. **Session launch saves working branch**: If no working branch is specified, `session_create` auto-generates one as `swarmer/session-{id}-{hex}` after the initial commit (requires a second commit).

10. **Shared `_do_launch()` function**: Session launch logic is in `routers/sessions.py:_do_launch()` — used by both the HTTP endpoint and the cron scheduler. The scheduler imports it at call time to avoid circular imports.

11. **Manual migrations**: New columns are added via `database.py:migrate_db()` with `ALTER TABLE` statements. Only "duplicate column" / "already exists" errors are suppressed; other failures re-raise so startup fails visibly. When adding a new column to an existing table, add the migration there and include a `server_default` so existing rows work.

12. **Blocking K8s calls in async handlers**: All synchronous `kubernetes` client calls inside async functions must be wrapped with `asyncio.to_thread()` to avoid blocking the event loop. The TUI WebSocket handler uses a background thread with `threading.Event` for the pod exec stream reader.

13. **`OpencodeSecret` naming is misleading**: Despite the name, this model stores credentials for all agent tools (OpenCode, Crush), including Anthropic and OpenAI API keys. The table name `opencode_secrets` is a legacy artifact.

14. **HX-Trigger pattern for repo management**: Repo add/delete endpoints return empty `HTMLResponse` with `HX-Trigger: repoListChanged` header. The template listens for this event to refresh the repo items partial via a separate GET endpoint.

15. **Chat proxy HTML rewriting**: For in-cluster OpenCode server sessions, the proxy injects a `<base>` tag and rewrites absolute asset paths (`src="/..."` → `src="/workspaces/{ws_id}/sessions/{sid}/chat/..."`). Crush sessions skip this and render a custom chat template instead.

16. **`image-build` requires `sync-images`**: The `image-build` Makefile target depends on `sync-images`, which reads `../agent-containers/.push-defaults`. If that file doesn't exist, the build fails. Use `SILENT=1` to skip the interactive version prompt.

17. **Container image runs as non-root**: The Containerfile uses UBI10 `python-312-minimal` with UID 1001. Directories `/data` and `/auth` are created as root then ownership dropped. PVCs must be group-0 writable for the non-root user.
